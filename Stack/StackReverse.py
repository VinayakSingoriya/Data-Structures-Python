...
def reverse(stack):                   # Recursive function that reverses the given stack using insertAtBottom()
    if (isEmpty(stack)):
        return
    temp = pop(stack)
    reverse(stack)
...
def insertAtBottom(stack, item):      # Recursive function that inserts an element at the bottom of a stack
    if isEmpty(stack):
        ...
        topElem  = pop(stack)         #Pop elements from stack and keep it in a function call
